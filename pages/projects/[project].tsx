...
export const getStaticPaths: GetStaticPaths = async () => {
  const projects = await axios
    .get<ProjectDetail[]>(`${apiProjects}projects`)
    .then((res) => res.data)
    .catch((error: unknown) => {
      console.error('Erreur récupération des projets pour getStaticPaths', error);
...
export const getStaticProps: GetStaticProps<ProjectPageProps> = async (context) => {
  const projectId = context.params?.project as string;

  const projectData = await axios
    .get<ProjectDetail[]>(`${apiProjects}project/${projectId}`)
    .then((res) => res.data)
    .catch((error: unknown) => {
      console.error('Erreur récupération du projet', error);
      return null;
    });
  ...
  return { props: { project: projectData[0] } };
};
