...
interface SelectProjectProps {
  listProject: SelectOption[];
  filteredProjects: (selected: SelectOption[]) => void;
}

const STORAGE_KEY = 'projects:selected-tags';
...
const customStyles: StylesConfig = {
  control: (base, state) => ({
    ...
    boxShadow: state.isFocused
      ? '0 0 0 3px rgba(242, 209, 166, 0.22)'
      : '0 10px 30px -20px rgba(0, 0, 0, 0.85)',
...
export default function SelectProject({ listProject, filteredProjects }: SelectProjectProps) {
  const animatedComponents = useMemo(() => makeAnimated(), []);
  const [valueProjectSelected, setValueProjectSelected] = useState<SelectOption[]>([]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedValues: unknown = JSON.parse(raw);
      if (!Array.isArray(savedValues)) return;

      const restoredSelection = listProject.filter((option) =>
        (savedValues as string[]).includes(option.value),
      );
  ...
  const handleSelectChange = (selectedValues: SelectOption[] | null) => {
    ...
    try {
      ...
      window.localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify(nextSelection.map((item) => item.value)),
      );
    } catch (error) {
