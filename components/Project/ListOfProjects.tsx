...
interface ProjectSection extends SectionRule {
  projects: ProjectDetail[];
}

interface ListOfProjectsProps {
  projectsFilter: ProjectDetail[];
}
...
const projectMatchesSection = (project: ProjectDetail, section: SectionRule): boolean => {
  const normalizedProjectTags = (project.tags ?? []).map(normalizeTag);
  return normalizedProjectTags.some((tag) => section.tags.includes(tag));
};

const getProjectSections = (projectsFilter: ProjectDetail[]): ProjectSection[] => {
  const categorizedProjectIds = new Set<string>();

  const sections: ProjectSection[] = SECTION_RULES.map((section) => {
  ...
  const uncategorizedProjects = projectsFilter.filter(
    (project) => !categorizedProjectIds.has(project._id),
  );
...
export default function ListOfProjects({ projectsFilter }: ListOfProjectsProps) {
  const rowRefs = useRef<Record<string, HTMLDivElement>>({});

  const normalizedProjects = Array.isArray(projectsFilter) ? projectsFilter : [];
  const sections = getProjectSections(normalizedProjects);

  const setRowRef = useCallback((sectionKey: string, element: HTMLDivElement | null) => {
    if (element) {
      rowRefs.current[sectionKey] = element;
    }
  }, []);

  const handleScrollRow = useCallback((sectionKey: string, direction: 'left' | 'right') => {
